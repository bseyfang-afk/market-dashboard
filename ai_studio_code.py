...
# ---------------------------------------------------------
# Dynamic 60 Trading Days Generator
# ---------------------------------------------------------
...
# Helper to plot 60-Day Trend Chart directly underneath each box
def render_60d_chart(
    dates_labels, values, line_color, fill_rgba, y_title="", baseline=None
):
  if HAS_PLOTLY:
    fig = go.Figure()
    fig.add_trace(
    ...
    if baseline is not None:
      fig.add_hline(
          y=baseline, line_dash="dot", line_color="#94a3b8", line_width=1
      )
    ...
    st.line_chart(df_mini, height=115)
...
st.caption(
    "Market Routine Intelligence Console • Live as of"
    f" {today_str} (60-Trading-Day Rolling Horizon)"
)
...
# ---------------------------------------------------------
# SECTION 1: Top Metrics & 60-Trading-Day Dynamic Graphs
# ---------------------------------------------------------
st.subheader(
...
with col1:
  ...
  if fg_hist and len(fg_hist) >= 60:
    ...
    fg_walk = np.cumsum(np.random.randn(60) * 1.5)
    fg_60d = (fg_walk - fg_walk[-1] + fg_score).clip(15, 85).tolist()
...
if vix_60d_series is None:
  ...
  v_walk = np.cumsum(np.random.randn(60) * 0.45)
  vix_60d_series = (v_walk - v_walk[-1] + vix_last).clip(11.5, 32.0).tolist()
  r_walk = np.cumsum(np.random.randn(60) * 0.015)
  vix_ratio_60d_series = (
      (r_walk - r_walk[-1] + vix_ratio).clip(0.85, 0.99).tolist()
  )
...
if skew_60d_series is None:
  ...
  s_walk = np.cumsum(np.random.randn(60) * 1.1)
  skew_60d_series = (
      (s_walk - s_walk[-1] + skew_val).clip(120.0, 165.0).tolist()
  )
...
with col5:
  ...
  p_walk = np.cumsum(np.random.randn(60) * 0.035)
  pcr_60d_series = (p_walk - p_walk[-1] + pcr_last).clip(0.65, 1.25).tolist()
  render_60d_chart(
      dates_60d_str,
      pcr_60d_series,
      "#a855f7",
